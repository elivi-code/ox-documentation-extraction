...
- Screen readers do not announce the border change. Always pair with a visible error message linked via `aria-describedby` and `role="alert"`.
...
| 1.4.3 Contrast (Minimum) | AA | ✅ Passes | Label `#26252a` on white, placeholder `#6c6862` on `#f4f3ee` — review contrast for disabled state (`#8d8b7e` on `#c8c8bd`) |
...
| 3.3.1 Error Identification | A | ⚠️ Consumer responsibility | `hasError` adds visual border; consumer must provide accessible error message via `aria-describedby` + `role="alert"` |
...
_Source: Oxygen MCP + Figma MCP (accessibility annotations not present in Figma — guidance inferred from component type) · Extracted 2026-04-29_
