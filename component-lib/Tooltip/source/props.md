...
| `disableInteractive` | `boolean` | No | `false (inferred)` | When true, the tooltip dismisses immediately when the cursor leaves the trigger (not interactive) |
| `disableDescribedBy` | `boolean` | No | `false (inferred)` | When true, suppresses the `aria-describedby` binding on the trigger |
