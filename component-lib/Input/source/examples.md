...
The Search input is documented in Figma as a dedicated component set (node `25655:40530`) with a leading search icon. In the OX API it maps to `Input` with `iconLeft` and `type="search"`:
...
> **Note (WARN-003):** Search input has no separate OX package — it maps to `Input + iconLeft + type="search"`. Whether `isReadOnly` applies to Search input is unresolved (CONFLICT-001 in [Input-audit.md](../Input-audit.md)).
...
## With prefix (autoPrefixWidth)

The `autoPrefixWidth` prop auto-sizes the leading prefix container to its content. Confirm the `prefix` prop name against `@8x8/oxygen-input` source (it is not returned by the OX MCP — tracked as GAP-002):

```tsx
<Input
  autoPrefixWidth
  inputProps={{ type: 'text', placeholder: 'Value', id: 'prefixed-field' }}
/>
```
