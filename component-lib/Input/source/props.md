...
| `focus` | `boolean` | No | `false` | Programmatically force focused state |
| `fixed` | `boolean` | No | `false` | Fixed width mode |
| `hasError` | `boolean` | No | `false` | Puts the input in error state (red border + error styling) |
| `isDisabled` | `boolean` | No | `false` | Disables the input — no interaction, visually muted |
| `isReadOnly` | `boolean` | No | `false` | Makes input read-only — shows value but not editable |
| `isRequired` | `boolean` | No | `false` | Marks the field as required |
| `fullWidth` | `boolean` | No | `false` | Stretches the input to 100% of its container width |
...
| `autoSuffixWidth` | `boolean` | No | `false` | Automatically sizes the suffix width to content |
| `autoPrefixWidth` | `boolean` | No | `false` | Automatically sizes the prefix width to content |
