...
const SmallPagination = ({ totalRecords }: { totalRecords: number }) => {
  const [pagination, setPagination] = useState<PaginationState>({
    pageNumber: 1,
    rowsPerPage: 10,
  });
...
When the backend cannot return a total count (cursor-paginated APIs, streamed results), hide the "of N pages" label by passing an empty string to `translations.numberOfPages`.

```tsx
import React, { useState } from 'react';
import Pagination, { usePagination, PaginationState } from '@8x8/oxygen-pagination';

const UnknownPagesPagination = ({
  hasPrev,
  hasNext,
}: {
  hasPrev: boolean;
  ...
  return (
    <Pagination
      ...
      translations={{
        rowsPerPage: 'Rows per page:',
        prevPage: 'Prev',
        numberOfPages: '',   /* empty string hides the "of N pages" label */
        nextPage: 'Next',
      }}
    />
  );
