...
All table primitives, hooks, and helpers below are exported from the `dataTable` package entry point. The symbols shown are those referenced across the examples on this page:
...
> Icons used in the row-actions example (`PencilIcon`, `LeaveIcon`, `GearIcon`) come from the Oxygen icon package, not `dataTable`. `Person`, `createPersonColumns`, `usePersonMockApi`, `useMockGetData`, and `paginationTranslations` are user-supplied test fixtures (see [GAP-006 in the audit](DataTable-audit.md)).
