...
<!-- SKIP:GAP-011 manual="All guidance is inferred from component type (passive/display). Verify the rendered DOM of SkeletonCircle in a browser; confirm prefers-reduced-motion CSS behaviour; update to reflect actual @8x8/oxygen-skeletons implementation." -->

> **DOC_GAP (GAP-011):** The Oxygen MCP returned no explicit accessibility documentation for `SkeletonCircle`. No ARIA annotations were found in Figma. Guidance below is inferred from WCAG 2.1 AA requirements and component type (purely presentational).
