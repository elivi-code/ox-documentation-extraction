...
- Also set `aria-invalid="true"` — this is the standard ARIA attribute for marking an invalid form field. Screen readers announce it as "invalid entry" or equivalent before the field label.
...
| 1.4.3 Contrast (Minimum) | AA | ✅ Passes | Label `#26252a` on white, placeholder `#6c6862` on `#f4f3ee` pass AA. Disabled text `#8d8b7e` on `#c8c8bd` ≈ 1.9:1 — below 3:1 normal text threshold, but disabled states are **explicitly exempt** from WCAG 1.4.3 (SC Note 1: "Text or images of text that are part of an inactive user interface component…have no contrast requirement") |
